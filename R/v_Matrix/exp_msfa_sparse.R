...
)
{
  ...
  inv_W <- Map( WoodburyMatrix::WoodburyMatrix, A = inv_Psi_s, B = I_j, X = test$Lambda_s )
  W <-  lapply(inv_W, solve)
  Sig_s <- Map( WoodburyMatrix::WoodburyMatrix, A = W, B = list(I_k), X = list(Phi) )

  delta_Phi_t <- Map( solve, t(Sig_s), list(test$Phi) )
  delta_Lambda_t <- Map( solve, t(Sig_s), test$Lambda_s )

  delta_Phi <- lapply(delta_Phi_t, t)
  delta_Lambda <- lapply(delta_Lambda_t, t)
